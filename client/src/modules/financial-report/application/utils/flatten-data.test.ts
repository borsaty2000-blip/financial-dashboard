...
describe('flattenData', () => {
	it('flattens nodes in preorder', () => {
		expect(flattenFinancialReportData(input).map((row) => row.id)).toEqual([
			'company',
			'branch-1',
			'employee',
			'channel',
	...
	it('assigns the correct depth and ancestor ids', () => {
		expect(
			flattenFinancialReportData(input).map(({ id, depth, ancestorIds }) => ({
				id,
				depth,
				ancestorIds,
			})),
	...
	it('marks only nodes with children as expandable', () => {
		expect(
			flattenFinancialReportData(input).map(({ id, hasChildren }) => ({
				id,
				hasChildren,
			})),
		).toEqual([
