...
def _run(prices: list[float], signal: Callable[[np.ndarray], int], strategy: str, lookback: int = 30, horizon: int = 7) -> dict[str, Any]:
    ...
    for index in range(lookback, len(values) - horizon):
        ...
        if direction == 0:
            continue
        realized = direction * (values[index + horizon] / values[index] - 1)
        returns.append(float(realized))
        equity.append(equity[-1] * (1 + float(realized)))
        trades.append({"entry_index": index, "exit_index": index + horizon, "direction": "long" if direction > 0 else "short", "return": round(float(realized * 100), 6)})
    return _metrics(returns, equity, trades, strategy, horizon)


def backtest_elliott(prices: list[float], lookback: int = 30, horizon: int = 7) -> dict[str, Any]:
    ...
    return _run(prices, signal, "elliott", lookback, horizon)


def backtest_gann(prices: list[float], lookback: int = 30, horizon: int = 7) -> dict[str, Any]:
    def signal(window: np.ndarray) -> int:
    ...
    return _run(prices, signal, "gann", lookback, horizon)
...
def backtest_indicators(prices: list[float], strategy: str = "rsi_macd", lookback: int = 30, horizon: int = 7) -> dict[str, Any]:
    if strategy != "rsi_macd":
        raise ValueError("unsupported strategy; use rsi_macd")
    return _run(prices, _indicator_signal, strategy, lookback, horizon)
