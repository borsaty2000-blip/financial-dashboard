...
## ملحق الجولة الهندسية الشاملة — إصلاحات التسجيل والتحليل

أُجري مسح فعلي لصفحات الإنتاج و74 مسار API. ثبت أن HTML وJavaScript وCSS تُحمّل، وأن صفحات التسجيل والدخول والسوق ومنشئ الاستراتيجيات تُرسم دون أخطاء Console أولية. عولجت صفحة تفاصيل السهم كي لا تبقى في skeleton عند تأخر خدمة الشموع؛ أصبحت تنهي الطلب بمهلة واضحة وتعرض حالة «بيانات السهم غير متاحة حالياً» بدلاً من ترك مساحة فارغة.

كان نموذج التسجيل يجمع تفضيلات المستخدم ثم يسقطها قبل إرسالها، ويقرأ قبول الشروط من DOM، ولا يمنع الإرسال المكرر. أُصلح العقد ليقبل تفضيلات الأسواق والخبرة وأسلوب الاستثمار والوقت اليومي بعد التحقق، وتحفظ مع إنشاء المستخدم داخل المعاملة نفسها، مع تحقق أقوى للبريد وكلمة المرور وحالة إرسال واضحة وروابط الشروط وإخلاء المسؤولية. لا يمكن اعتبار إنشاء الحساب في الإنتاج ناجحاً قبل إصلاح اتصال Neon في Vercel؛ لم يُنشأ حساب تجريبي ولم تُغيّر قاعدة بيانات الإنتاج.

كان غياب Python service في Vercel يؤدي إلى 502 في الإحصاء والتنبؤ والensemble والمشاعر والشذوذ والbacktesting. أضيفت fallbacks Node حتمية ومعلنة تعليمياً، ولا تدّعي تشغيل النموذج الأصلي ولا تنتج بيانات سوق أو توصيات مختلقة. كما أُصلح دعم الأسعار اليدوية في anomalies، وحُوّل فشل فحص Halal إلى `available:false` واضح بدلاً من 502. اختبار تكامل محلي مع Python URL غير متاح أعاد 200 لجميع هذه المسارات باستخدام البدائل الآمنة.
...
## ملحق بيانات السوق — ربط المزودين والتحقق الفعلي

تم اختبار مفاتيح Twelve Data وSAHMK وFinnhub محلياً دون كشف القيم السرية. أعاد Twelve Data دليل EGX الفعلي الذي يضم 265 ورقة، وأعاد أسعار المعادن، بينما أعاد SAHMK ملخص TASI وسعر سهم سعودي مع احترام حقل `is_delayed`. كما أعاد Yahoo Finance شموعاً فعلية لـCOMI و2222 وبيانات مؤشرات ومعادن كـfallback متأخر.

أظهر الاختبار أن مفتاح Twelve Data الحالي لا يملك صلاحية أسعار/شموع EGX المطلوبة؛ فالـAPI يعيد قيد خطة Pro/Venture عند طلب معرف CIB الرسمي. لذلك أضيف تطبيع دليل الشركات مع fallback موثق، ولا تُعرض أي قيمة مصطنعة أو وسم Live غير مثبت. أضيف polling للـquote كل 30 ثانية في الواجهة، وتمرير سوق TASI للرموز الرقمية، وتوحيد `source` و`freshness` و`delay_minutes`. البث اللحظي الكامل لـEGX يتطلب ترقية مزود مرخص أو ربط مزود EGX دائم، ولا يمكن اعتباره منجزاً بالمفاتيح الحالية وحدها.

## ملحق الجولة الهندسية — ربط البيانات والمساعد والقوائم

تمت إضافة عقد موحد `GET /api/v1/market/live` يجمع مؤشرات EGX30 وEGX70 وEGX100 وTASI والذهب والفضة، ودليل أسهم EGX والأخبار، مع إرجاع `freshness` و`source` و`timestamp` و`delayedByMinutes` وعدم اختلاق quote عند غياب المصدر. أصبح endpoint `POST /api/ai/chat` مساعداً عربياً حتمياً مرتبطاً بالشموع والتحليل الموجودين، ويعرض الإجماع ونتائج Elliott وGann ضمن إخلاء تعليمي صريح، بلا تنفيذ صفقات أو وعود استثمارية.

أعيد ربط الصفحة العامة بعقد السوق الموحد، وأضيف دليل أسهم EGX وحالتا فراغ وتحميل واضحتان، كما أضيفت قوائم منسدلة للأسواق والتحليل وروابطها الفعلية. أضيف Widget مساعد بورصتي إلى التخطيط العام، وأصبحت صفحة الأخبار تعرض حالات التحميل والخطأ والفراغ بدلاً من فشل صامت. اختُبرت المعاينة فعلياً: فتح قائمة الأسواق أعاد أربعة روابط، وفتح قائمة التحليل أعاد أربعة روابط، وفتح المساعد أظهر لوحة السؤال.

نجحت بوابات التحقق: `npm run typecheck`، `npm run typecheck:packages`، `npm run build`، اختبارات الواجهة (91 ناجحة و6 متخطاة لاختبارات مزود خارجي اختيارية)، `npm run test:server` (11 ناجحة)، `npm run test:auth` (5 ناجحة)، و`git diff --check`. أظهر الاختبار المحلي المؤرخ أن endpoint السوق الموحد يعيد بيانات حقيقية متأخرة عندما تكون متاحة؛ تظل البيانات اللحظية الحقيقية مشروطة بتفعيل ترخيص ومفتاح مزود يدعم البث اللحظي، ولا تعتبر Twelve Data لحظية ما لم يُضبط `TWELVE_DATA_REALTIME=true` بعد التحقق من الخطة.

هذه الجولة محلية فقط ولم تُدفع إلى GitHub أو تُنشر، التزاماً بقاعدة عدم النشر دون موافقة صريحة جديدة.

## ملحق المرحلة — دليل الأسهم المصرية والسعودية الكامل

تم توسيع طبقة دليل السوق بحيث يعيد endpoint `GET /api/market/egx/companies` كامل دليل EGX بدلاً من قصه إلى 60 شركة، كما أضيف fallback سعودي يعتمد على دليل Twelve Data لبورصة `XSAU` عندما لا يتوفر SAHMK. يعيد endpoint `GET /api/market/tasi/companies` envelope بحالة `delayed` وتأخير دلالي قدره 1440 دقيقة لأن تغطية XSAU لدى Twelve Data موصوفة رسمياً بأنها EOD؛ لا يتم تقديمها للمستخدم كبيانات لحظية.

أصبح endpoint `GET /api/v1/market/live` يعيد `directories.egx` و`directories.tasi` بالإضافة إلى الحقل القديم `companies` للحفاظ على التوافق. أضيفت الصفحة الرئيسية جدولين كاملين قابلين للبحث بالرمز أو اسم الشركة، مع فتح صفحة تفاصيل السهم عند الضغط على الرمز. تعرض الأسعار والتغيرات فقط إذا أعاد مصدر موثوق قيمة فعلية، وتعرض `—` خلاف ذلك؛ لا تظهر أوقات التحديث في الواجهة، وتبقى freshness ممثلة داخلياً بنقطة لون فقط.

نجحت بوابات المرحلة: `npm run typecheck`، و`npm run typecheck:packages`، واختبارات الخادم بما فيها عقد XSAU، واختبارات المصادقة، واختبارات الواجهة (91 اختباراً ناجحاً و6 اختبارات مزود خارجية متخطاة عند غياب إعدادها)، و`npm run build`. لم يتم دفع هذه المرحلة أو نشرها بعد.

المصدر المرجعي لتغطية XSAU وكونها EOD موثق في `docs/qa/market-directory-source-notes-2026-09-14.md`.

## ملحق المرحلة — الأرقام الإنجليزية ودمج أنظمة التحليل

تم توحيد الأرقام المالية المعروضة في الواجهة إلى أرقام لاتينية عبر منسق مشترك، مع إبقاء العربية للنصوص واتجاه RTL، وإظهار `—` للقيم غير المتاحة. شمل التغيير الصفحة الرئيسية، تفاصيل السهم، الحساب، الأخبار، التقويمات، المحاكاة وسجل الأمان، مع اختبار وحدات مخصص للتجميع والدقة والنسب.

أضيفت لوحة تحليل مدمج في صفحة تفاصيل السهم تجمع المؤشرات الفنية، Elliott، Gann، Consensus، الإحصاء والمخاطر، ARIMA/LSTM التعليمية، وBacktest التعليمي، إلى جانب Ensemble والمشاعر والشذوذ الموجودة. تُحمّل المحركات بشكل مستقل، ولا يؤدي فشل محرك واحد إلى إسقاط بقية اللوحة. لا تنفذ اللوحة صفقات ولا تقدم ضمانات استثمارية.

نجحت بوابات المرحلة: `npm run typecheck`، `npm run typecheck:packages`، `npm test -- --run` بنتيجة 94 اختباراً ناجحاً و6 متخطاة لاختبارات المزود الخارجي الاختيارية، `npm run test:server` بنتيجة 12 اختباراً ناجحاً، `npm run build`، وESLint مستهدف للملفات الجديدة. كما نجح smoke test المحلي بعد تشغيل نسخة خادم نظيفة: ثمانية من أصل ثمانية مسارات للتحليل المدمج عادت HTTP 200، بما فيها fallbacks الإحصائي والتنبؤي والـBacktest. التفاصيل في `docs/qa/phase-english-numerals-integrated-analysis-2026-09-14.md`.

هذه المرحلة محلية فقط ولم تُدفع إلى GitHub أو تُنشر بعد.

## ملحق المرحلة — واجهة القوائم ودليل الأسهم الاحترافي

تم إصلاح ألوان القوائم المنسدلة في رأس الصفحة باستخدام أسطح واضحة opaque، وتباين صريح، وحالات hover وfocus، ودعم الوضع الداكن، مع إضافة `aria-haspopup` و`role="menu"` و`role="menuitem"`. كما أضيف سهم متحرك يوضح حالة فتح القائمة.

أعيد تنظيم دليل EGX وTASI ليشمل البحث والترتيب والترقيم بمعدل 25 صفاً في الصفحة، مع عدادات إجمالي الشركات والأسعار المتاحة. يعرض كل صف الرمز المختصر عند توفره، والمعرّف الرسمي عند اختلافه، ويفتح صفحة التفاصيل باستخدام المعرّف القابل للحل. لا تعرض الواجهة أسماء المزودين أو أوقات التحديث، وتستخدم نقطة لون وحالة مختصرة «متاح» أو «دليل فقط».

على مستوى العقد، تُدمج الأسعار التي أعادتها الاستعلامات المحدودة داخل الصفوف الكاملة للدليل. أضيف حقل `coverage` داخلياً لقياس إجمالي الشركات وعدد الصفوف المسعّرة، مع إبقاء جلب EGX وTASI محدوداً إلى 12 رمزاً لكل سوق في الطلب الواحد. لا تُملأ الصفوف التي لم تُرجع سعراً بقيم تقديرية، ويثبت اختبار تعاقدي جديد أن `null` يبقى `null` عند غياب quote حقيقي.

أعاد الإنتاج قبل النشر HTTP 200 مع 265 شركة EGX و531 شركة TASI و10 صفوف أسعار في `topMovers`. وفي المعاينة المحلية ظهر دليل EGX كامل من 265 شركة، مع 25 صفاً في الصفحة الأولى، و10 أسعار متاحة، و255 صفاً مصنفاً «دليل فقط». نجحت `npm run typecheck`، واختبارات الواجهة بنتيجة 94 ناجحة و6 متخطاة، واختبارات الخادم بنتيجة 13 ناجحة، و`npm run build`، وPrettier، وESLint لملف الصفحة. فشل lint العام السابق بسبب اكتشاف إعداد ESLint خارج نطاقه، وليس بسبب خطأ في الملف المعدل؛ فحص الصفحة مباشرة من إعداد client نجح.

تم دفع هذه المرحلة إلى GitHub ونشرها في الإنتاج بعد اجتياز التحقق. التفاصيل والقياسات في `docs/qa/phase-market-ui-directory-2026-09-14.md`.

## ملحق المرحلة — الشريط العلوي والقوائم التفاعلية

تم تحويل الشريط العلوي إلى خمس Mega Menus فعلية: الأسواق، التحليل، الأخبار، الأدوات، والتعلم. قائمة الأسواق تعرض EGX وTASI والذهب والفضة وفاحص الأسهم والفوركس والأصول الرقمية، وتربط أعداد الشركات وقيم المؤشرات بالبيانات التي تحملها الصفحة. قائمة التحليل تعرض Elliott Wave وGann والاختبار التاريخي ومنشئ الاستراتيجيات ومقارنة الأسهم والشموع والمؤشرات. قائمة الأخبار تعرض آخر ثلاثة عناوين حقيقية عند توفرها أو حالة صريحة عند غياب الخدمة. وتضم قائمتا الأدوات والتعلم المحفظة وقوائم المتابعة والتنبيهات والتقويم والأكاديمية والفيديوهات والندوات والمجتمع والمتصدرين.

أضيف دعم لوحة المفاتيح عبر Enter وArrowDown وEscape، مع `aria-expanded` و`aria-haspopup` وأدوار `menu` و`menuitem`. صُممت القوائم للوضعين الفاتح والداكن، وتحتوي على وصف وإحصاءات وروابط موجهة إلى مسارات موجودة فعلياً، وليست عناصر وهمية أو روابط ميتة.

نجحت بوابات المرحلة: `npm run typecheck`، واختبارات الواجهة بنتيجة 94 ناجحة و6 متخطاة، و`npm run build`، وESLint لصفحة `PublicPages.tsx`، وPrettier. في المعاينة البصرية ظهرت القوائم الخمس، وظهرت ستة عناصر فعلية في قائمة التحليل، وظهرت حالة الأخبار المحلية الحقيقية بلا عناوين مصطنعة. التفاصيل في `docs/qa/phase-top-navigation-mega-menu-2026-09-14.md`.

تم دفع هذه المرحلة إلى GitHub ونشرها في الإنتاج بعد اجتياز التحقق.

## ملحق المرحلة — الصفحة الرئيسية الغنية ولوحة نبض السوق

تمت ترقية الصفحة الرئيسية بإضافة لوحة «نبض السوق» بعد مؤشرات الصفحة. تحتوي اللوحة على تبويبات للنظرة العامة، الأسهم المصرية، الأسهم السعودية، والذهب والفضة، وبطاقات للمؤشرات، وجدول «الأكثر حركة» مع فرز حسب التغير أو السعر. كل صف يعرض الرمز والاسم والسعر والتغير عند توفره، وينقل المستخدم إلى صفحة تفاصيل السهم عند النقر.

تعتمد اللوحة على `live.indices` و`live.directories` الفعلية، ولا تعرض إلا الأسعار التي أعادها العقد. القيم غير المتاحة تبقى `—`، ولا تُضاف أرقام تجريبية. كما تم منع استجابات 304 المخزنة من إخفاء التحديثات عبر استخدام `cache: 'no-store'` و`cache-control: no-cache` في جلب بيانات الصفحة.

في المعاينة المحلية ظهرت أرقام EGX30 وEGX70 وEGX100 وTASI والذهب والفضة، وأظهر تبويب الأسهم المصرية 8 أسعار متاحة في العرض الحالي مع صفوف قابلة للنقر. نجحت `npm run typecheck`، واختبارات الواجهة بنتيجة 94 ناجحة و6 متخطاة، و`npm run build`، وESLint للصفحة والمكوّن الجديد، وPrettier. التفاصيل في `docs/qa/phase-rich-home-market-pulse-2026-09-14.md`.
تم دفع هذه المرحلة إلى GitHub ونشرها في الإنتاج بعد اجتياز التحقق.

## ملحق المرحلة — احترافية القوائم وروابط التنقل

تمت مراجعة جميع روابط Mega Menu العلوية مقابل تعريفات المسارات الفعلية في `client/src/App.tsx`. أضيفت أزرار انتقال واضحة داخل البطاقات التعريفية للأسواق والتحليل والتعلم، ورُفع عرض القائمة إلى 760px على سطح المكتب مع أعمدة مرتبة وبطاقات روابط وحالات Hover وFocus واضحة. كما أضيف `aria-controls`، وأصبح الضغط خارج القائمة أو زر Escape يغلقها مع الحفاظ على دعم Enter وArrowDown.

في اختبار المتصفح ظهرت قائمة الأسواق متعددة الأعمدة بستة روابط وزر «فتح مركز الأسواق». أدى النقر إلى فتح `/markets/egx` فعلياً، وظهرت صفحة «السوق المصري بواجهة أوضح» مع المؤشرات ودليل EGX. نجحت `npm run typecheck` وESLint المستهدف و`npm run build`. التفاصيل في `docs/qa/phase-navigation-links-polish-2026-09-14.md`.

تم دفع هذه المرحلة إلى GitHub ونشرها في الإنتاج بعد اجتياز التحقق.

## ملحق المرحلة — الأخبار الحديثة الموثقة

أضيفت طبقة fallback لخدمة الأخبار لأن خلاصات RSS الخارجية كانت قد تعود فارغة، مما يترك الصفحة الرئيسية بلا مقالات. تظل RSS الحية ذات الأولوية، وعند فشلها تظهر أربع مواد حديثة تم التحقق من صفحاتها الأصلية: جلسة EGX في 14 سبتمبر، إغلاق TASI، تقرير Reuters عن الأسهم السعودية والمخاطر الإقليمية، وحركة الفضة مع توقعات الفائدة. تحتوي كل مادة على مصدر ورابط وتاريخ نشر، ولا تُعامل كبيانات لحظية أو كتوصية استثمارية. نجح `npm run typecheck` واختبار الخادم بنتيجة 13/13، وأعاد endpoint المحلي أربع مواد موثقة. التفاصيل في `docs/qa/phase-current-news-fallback-2026-09-15.md`.

تم دفع هذه المرحلة إلى GitHub ونشرها في الإنتاج بعد اجتياز التحقق.

## ملحق المرحلة — محتوى الصفحات حسب نشاط القسم

أضيف مكوّن `SectionActivityBar` مشترك يربط كل صفحة بنشاطها الفعلي. صفحات الأسواق تعرض روابط EGX وTASI والفاحص والسلع، وصفحات التحليل تعرض Elliott وGann وBacktest والمقارنة، وصفحة الأخبار تعرض الأخبار والتقويم والأرباح والتوزيعات، وصفحات التعلم تعرض الأكاديمية والفيديو والندوات والمجتمع، ومساحة العمل تعرض القوائم والتنبيهات والمحفظة والتقرير الأسبوعي. يعمل المكوّن عبر router الداخلي، ويدعم RTL والوضع الداكن والهواتف ولوحة المفاتيح. نجحت اختبارات TypeScript وESLint واختبارات الواجهة وبناء الإنتاج. التفاصيل في `docs/qa/phase-contextual-section-content-2026-09-15.md`.

تم دفع هذه المرحلة إلى GitHub ونشرها في الإنتاج بعد اجتياز التحقق.

## ملحق المرحلة — دليل الأسهم العربي وربط التحليل

تمت ترقية دليل EGX وTASI إلى عرض عربي كثيف مع شرائح «جميع الأسهم» و«الأكثر ارتفاعاً» و«الأكثر نشاطاً»، وأسماء عربية موثقة للرموز الرئيسية، ورموز لاتينية، وأرقام أسعار وتغير إنجليزية. أصبح لكل صف زر يفتح صفحة السهم التي تجمع المؤشرات وElliott وGann وConsensus والإحصاء والتنبؤ والاختبار التاريخي. الأسعار غير المتاحة تظل `—` دون أرقام مختلقة، والبحث والفرز والترقيم محفوظة. نجحت TypeScript وESLint واختبارات الواجهة وبناء الإنتاج. التفاصيل في `docs/qa/phase-arabic-stock-directory-analysis-2026-09-15.md`.

تم دفع هذه المرحلة إلى GitHub ونشرها في الإنتاج بعد اجتياز التحقق.

## ملحق المرحلة — غرفة التحليل الموحدة للسهم

تم توحيد تحليل السهم في عقد واحد يبدأ من الرمز الذي يبحث عنه المستخدم، ويتحقق من الشموع، ثم ينسق ثمانية محركات: المؤشرات الفنية، Elliott، Gann، الإجماع، الإحصاء، ARIMA، LSTM، والاختبار التاريخي. تعرض صفحة السهم مراحل التنفيذ وعدد المحركات المكتملة، مع الحفاظ على مبدأ عدم اختراع الأرقام عند غياب البيانات. اختبار COMI المحلي أعاد HTTP 200 وأكمل 8 من 8 محركات، ونجح Build واختبارات الخادم والواجهة.


## ملحق المرحلة — سلامة غرفة التحليل وإزالة البيانات التجريبية

في 15 سبتمبر 2026 تمت مراجعة تحليل السهم فعلياً على COMI في الإنتاج. أصبح عقد التحليل الموحد يعلن جودة البيانات وحدود الفورية ويثبت `NO_TRADE_DECISION`. تم تعديل الإجماع ليعرض ميلاً تعليمياً ومقياس اتفاق بين المحركات، لا احتمال نجاح ولا أمراً بالشراء أو البيع. كما أزيلت بيانات الحوكمة والإفصاحات اليدوية التجريبية، وأصبح غياب الإفصاح يعرض حالة عدم التوفر. نجحت اختبارات الخادم وعددها 22، وفحص TypeScript وبناء الإنتاج.


## ملحق المرحلة — تدقيق دقة تحليل السهم والهوية — 15 سبتمبر 2026

كشفت المراجعة أن غرفة التحليل كانت تقبل تاريخاً قصيراً جداً، وأن عداد المحركات قد يساوي المحركات المكتملة حتى عند استخدام fallback تعليمي. تم تشديد الحد الأدنى إلى 60 جلسة، وإضافة حالات منفصلة للمحركات المحسوبة والاحتياطية التعليمية وغير المتاحة، مع تنبيه صريح بأن المخرجات ليست توصية تنفيذية أو احتمال نجاح. كما أصبح اسم الشركة ينتقل من دليل السوق إلى رأس غرفة التحليل بجوار الرمز. تفاصيل المخاطر والقيود موثقة في `docs/qa/phase-stock-analysis-accuracy-audit-2026-09-15.md`.

نجحت بوابات التحقق المحلية: TypeScript، 22 اختبار خادم، 94 اختبار واجهة مع 6 اختبارات تكامل خارجية متجاوزة، وبناء الإنتاج. لا تزال دقة أي تحليل رهناً بتوفر 60 جلسة صحيحة، جودة بيانات السوق، وتوفر المحرك الخارجي؛ ولا ينبغي اعتبار أي مخرج ضماناً أو أمراً بالشراء أو البيع.


## ملحق المرحلة — إصلاح معرّفات الأسهم وغرفة التحليل — 15 سبتمبر 2026

ثبت التدقيق أن بعض روابط EGX كانت تمرر ISIN مثل `EGS38191C010` إلى محركات تحتاج رمزاً قابلاً للتداول مثل `ABUK`. تمت إضافة تحويل موحد داخل `CandlesService` ومحول Twelve Data، بحيث تُحل الهوية قبل جميع مصادر الشموع والاقتباسات وجميع محركات التحليل. كما أصبحت غرفة التحليل تعرض الرمز المحلول وحالات المحركات الفعلية، وتعرض التحليل الجزئي بوضوح عند فشل محرك اختياري بدلاً من ترك فشل صامت أو عداد مضلل.

في الاختبار المحلي أعاد `COMI` HTTP 200 مع 8/8 محركات، وأعاد `EGS38191C010` HTTP 200 بعد تحويله إلى `ABUK` مع 8/8 محركات. نجحت اختبارات الخادم وعددها 23، واختبارات الواجهة وعددها 94، وفحص TypeScript وبناء الإنتاج. بقيت البيانات غير المتاحة صريحة، ولم تُنشأ أرقام سوقية مصطنعة. التفاصيل في `docs/qa/phase-stock-identifier-analysis-2026-09-15.md`.


## ملحق المرحلة — Market Mood الموزون — 15 سبتمبر 2026

تم فصل مشاعر الأخبار عن مزاج السوق. كانت بطاقة المزاج تعرض صفراً عند غياب أخبار موثوقة حتى مع وجود إجماع ومؤشرات سعرية. أضيفت `SentimentService.calculateMarketMood` بحساب موزون من الإجماع 40%، RSI بنسبة 20%، MACD بنسبة 20%، والتغير السعري بنسبة 20%. يعيد endpoint التحليل الموحد `data.marketMood` مع الدرجة والتسمية وتوزيع إيجابي/سلبي ومكونات الحساب. عند غياب جميع المدخلات يعاد `available: false` بدلاً من صفر مضلل.

في اختبار COMI المحلي أعاد endpoint HTTP 200، وإجماعاً قدره 63، ومزاج سوق قدره 70 بتسمية «خير». نجحت اختبارات الخادم وعددها 25، واختبارات الواجهة وعددها 94، وفحص TypeScript وبناء الإنتاج. بقي المؤشر وصفياً تعليمياً وليس احتمال نجاح أو توصية تداول. تفاصيل الاختبار في `docs/qa/phase-market-mood-weighted-2026-09-15.md`.


## ملحق المرحلة — خريطة السوق الحرارية — 15 سبتمبر 2026

أضيفت صفحة `/heatmap` المرتبطة بقائمة الأسواق وشريط النشاط. تعتمد الخريطة على أسعار وتغيرات أعادها مزود السوق فعلياً، وتفتح بطاقة السهم غرفة التحليل مع السوق والاسم الصحيحين. لا تُملأ الأسهم التي لا تتوفر لها أسعار بأرقام وهمية، وتعرض الصفحة حالة عدم التوفر صراحة. جرى إبقاء عدد الرموز المطلوب محدوداً لحماية معدل الطلبات في بيئة Serverless؛ الدليل الكامل لكل أسهم EGX وTASI موجود في صفحات الأسواق، بينما تمثل الخريطة اللقطة السعرية المتاحة فقط. اجتازت المرحلة فحوص TypeScript، واختبارات الخادم والواجهة، وبناء الإنتاج، واختبار HTTP محلياً.
