...
## جولة الإصلاح النهائية المحلية

أغلقت هذه الجولة مسارات الواجهة الناقصة بإضافة aliases لـ`/learning` و`/leaderboard`، وأضفت حالات فراغ وخطأ صريحة لأكاديمية التعليم بدلاً من شاشة فارغة. أضيف timeout مدته 15 ثانية لعميل API، وتمت حماية lazy loading لمنشئ الاستراتيجيات بمهلة 15 ثانية وErrorBoundary يعرض إعادة تحميل كاملة برسالة عربية آمنة. تحقق المتصفح المحلي من `/learning` و`/leaderboard` و`/strategies`: ظهرت الصفحة أو حالة عدم التوفر، ولم يبق spinner بلا نهاية.

أُزيلت مسارات منح الإنجازات العامة `POST /api/achievements/award/:code` و`POST /api/achievements/check` لأن تسجيل الدخول وحده لم يكن إثباتاً لاستحقاق الإنجاز. أضيف اختبار عقدي يثبت أن هذه المسارات لا تمنح إنجازاً وتعيد 401 من الحماية العامة. أضيف حد JSON حجمه 256KB، وضبط `trust proxy`، و`Vary: Origin`، ومنع منح CORS للمصادر غير المسموحة، و`Retry-After` عند 429. كما تحولت refresh/reset tokens إلى SHA-256 hashes في التخزين؛ هذا يلغي صلاحية الجلسات والتوكنات القديمة بعد النشر ويستلزم تسجيل دخول جديداً.

بوابات الإصلاح المحلية: `npm run typecheck` ناجح، `npm run typecheck:packages` ناجح، `npm run build` ناجح، `npm run test:server` ناجح مع 9 اختبارات، `npm run test:auth` ناجح مع 5 اختبارات، واختبارات الواجهة ناجحة مع 19 ملفاً و91 اختباراً. نجح lint المستهدف للملفات المعدلة. بقي lint الكامل عند 153 مشكلة (149 error و4 warnings) بسبب دين موروث في ملفات واجهة أخرى، ولذلك لا تزال بوابة lint الكاملة غير ناجحة. `npm audit --omit=dev --audit-level=high` أظهر 3 High و0 Critical في `@prisma/config` و`prisma` و`deepmerge-ts`; لم يُنفذ إصلاح تلقائي.

هذه التغييرات لم تُدفع ولم تُنشر بعد. قرار العرض للمستثمر يظل **Conditional**: الرابط صالح لتجربة تقنية مشروطة، وليس لإثبات منصة بيانات حية أو traction أو جاهزية استثمارية كاملة. قبل النشر يجب مراجعة diff، تنفيذ smoke test على deployment الجديد، تفعيل مزودي البيانات بعقود وحقوق إعادة توزيع، وفصل migrations عن build مع backup/rollback.

## جولة ربط بيانات السوق — 14 سبتمبر 2026

تم اختبار مفاتيح `Twelve Data` و`SAHMK` و`Finnhub` محلياً دون طباعة أي قيمة سرية. نجحت طلبات Twelve Data للذهب وفهرس EGX، ونجح فهرس EGX في إعادة **265** شركة. كما نجح SAHMK في ملخص TASI وسعر سهم سعودي، ونجح Yahoo Finance في إعادة شموع COMI و2222.

أظهر الاختبار التفصيلي أن مفتاح Twelve Data الحالي يعيد قيد خطة واضحاً عند طلب سعر أو شموع CIB/EGX؛ لذلك لا نعرضه كبيانات لحظية. يعيد SAHMK الحقل `is_delayed` لسهم 2222 على أنه متأخر، ولذلك تمرر المنصة ذلك للمستخدم بدلاً من وسمه حيّاً. في المعاينة المحلية أعادت العقود HTTP 200، و265 شركة EGX، و30 شمعة COMI، و30 شمعة 2222، وسعر COMI حقيقياً من Yahoo مع `freshness=delayed`، وسعر 2222 من SAHMK مع `freshness=delayed`، والذهب من Twelve Data والفضة من Yahoo مع freshness متأخر.

أضيفت طبقة `twelve-data.adapter.ts` التي تجلب دليل EGX وتحوّل الرمز المختصر مثل `COMI` إلى معرف Twelve Data الرسمي عندما يكون متاحاً. وأضيف polling للواجهة كل 30 ثانية بدلاً من الاعتماد على Socket.io غير الملائم لـVercel Serverless، مع إيقاف polling عند إخفاء الصفحة. كما يستنتج Stock Detail سوق TASI للرموز الرقمية مثل `2222` ويمرر السوق إلى quote والشموع.

النتيجة التشغيلية: **الأسعار والشموع الحالية متاحة فعلياً عبر مزود حي أو fallback متأخر، لكن لا يجوز وصف كل EGX/TASI بأنها لحظية قبل تفعيل خطة بيانات مرخصة لذلك**. لا توجد أرقام بديلة مولدة من التطبيق.
